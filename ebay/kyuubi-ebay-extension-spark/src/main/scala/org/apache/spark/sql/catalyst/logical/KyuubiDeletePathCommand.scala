...
case class KyuubiDeletePathCommand(path: String, recursive: Boolean)
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    val toDelete = new Path(path)
    val fs = toDelete.getFileSystem(sparkSession.sessionState.newHadoopConf())
    val result = Seq(formatPathRow(fs.getFileStatus(toDelete)))
