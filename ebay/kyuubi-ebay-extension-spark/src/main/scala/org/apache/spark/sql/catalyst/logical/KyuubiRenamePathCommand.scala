...
case class KyuubiRenamePathCommand(path: String, dest: String, overwrite: Boolean)
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    val from = new Path(path)
    val destPath = new Path(dest)
    val fs = from.getFileSystem(sparkSession.sessionState.newHadoopConf())
    if (fs.exists(destPath)) {
      if (overwrite) {
        fs.delete(destPath, true)
      } else {
        throw KyuubiSQLException(s"Destination path $dest already exists")
      }
    }
    fs.rename(from, destPath)
    Seq(formatPathRow(fs.getFileStatus(destPath)))
  }
}
