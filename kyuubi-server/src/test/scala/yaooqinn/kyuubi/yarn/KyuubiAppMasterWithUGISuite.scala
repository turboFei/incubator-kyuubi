...
class KyuubiAppMasterWithUGISuite extends ZookeeperFunSuite with MockitoSugar {
  ...
  override def beforeAll(): Unit = {
    ...
    conf.set(KyuubiConf.APPMASTER_MEMORY.key, "10m")
    conf.set(KyuubiConf.APPMASTER_MEMORY_OVERHEAD.key, "1m")
    conf.set(KyuubiConf.APPMASTER_CORES.key, "1")
  ...
  test("test get lock path and instancePath") {
    val prefix = "/" + conf.get(KyuubiConf.APPMASTER_HA_ZOOKEEPER_NAMESPACE.key) + "/" +
      username + "/"
    assert(KyuubiAppMasterWithUGI.getLockPath(username, conf) ===
      prefix + HighAvailableService.LOCK)
