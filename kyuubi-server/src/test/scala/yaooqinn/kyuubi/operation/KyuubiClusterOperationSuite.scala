...
class KyuubiClusterOperationSuite extends OperationSuite {
  ...
  test("test execute kyuubiClusterOperation with exception") {
    val op = sessionMgr.getOperationMgr.newExecuteStatementOperation(session, "wrong statement")
      .asInstanceOf[KyuubiClusterOperation]
    val e = intercept[KyuubiSQLException](op.execute())
  }
  ...
  test("test kyuubiClusterOperation with exception") {
    ...
    var e = intercept[KyuubiSQLException](op.execute())
    assert(e.getSQLState === "08S01")
    e = intercept[KyuubiSQLException](op.getResultSetMetaDataResp())
    assert(e.getSQLState === "08S01")

    ReflectUtils.setSuperField(op, "state", FINISHED)
    e = intercept[KyuubiSQLException](op.getResultResp(FETCH_NEXT, 10, FetchType.QUERY_OUTPUT))
    assert(e.getSQLState === "08S01")
    e = intercept[KyuubiSQLException](op.getResultResp(FETCH_NEXT, 10, FetchType.LOG))
    assert(e.getSQLState === "08S01")
