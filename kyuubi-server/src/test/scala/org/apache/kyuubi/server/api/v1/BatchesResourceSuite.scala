...
abstract class BatchesResourceSuiteBase extends KyuubiFunSuite
  ...
  override def afterEach(): Unit = {
    ...
    sessionManager.getBatchesFromMetadataStore(MetadataFilter(), 0, Int.MaxValue).foreach {
      batch =>
  ...
  test("batch sessions recovery with metadata not reserved") {
    ...
    sessionManager.insertMetadata(batchMetadata)
    assert(sessionManager.getBatchFromMetadataStore(batchId1).get.getState.equals("PENDING"))
