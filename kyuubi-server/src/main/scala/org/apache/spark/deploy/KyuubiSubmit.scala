...
object KyuubiSubmit extends Logging {
  ...
  def submitCluster(args: Array[String]): ApplicationId = {
    preProcess(args)
    try {
      KyuubiYarnClient.startKyuubiAppMaster()
    } catch {
      case t: Throwable =>
        error(s"Error ocuured when starting Kyuubi with cluster mode.")
        throw t
    }
  }
  ...
  def prepareSubmitEnvironment(args: SparkSubmitArguments): (Seq[String], Map[String, String]) = {
    ...
    args.deployMode match {
      case "client" =>
      case "cluster" =>
      case _ => printWarning("Kyuubi only supports client and cluster mode," +
        " failover to default client mode.")
