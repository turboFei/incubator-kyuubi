...
class AuthenticationFilter(conf: KyuubiConf) extends Filter with Logging {
  ...
  override def doFilter(
    ...
    if (matchedHandler == null) {
      ...
      HTTP_CLIENT_IP_ADDRESS.set(
        Option(httpRequest.getHeader(conf.get(KyuubiEbayConf.HTTP_PROXY_CLIENT_IP_HEADER)))
          .filter(_.nonEmpty).getOrElse(httpRequest.getRemoteAddr))
      try {
