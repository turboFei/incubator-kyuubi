...
class KyuubiAmSparkSessionCacheManager private(name: String)
  extends SparkSessionCacheManager(name) {
  ...
  private val timesCount: AtomicInteger = new AtomicInteger()
  @volatile
  ...
  def this() = this(classOf[KyuubiAmSparkSessionCacheManager].getSimpleName)
  ...
  private val sessionCleaner = new Runnable {
    override def run(): Unit = {
      (sparkSession, timesCount) match {
        case (Some(ss), _) if ss.sparkContext.isStopped =>
          warn(s"SparkSession for $user might already be stopped by forces outside Kyuubi.")
          sparkSession = None
        case (Some(_), times) if times.get() > 0 =>
          debug(s"There are $times active connection(s) bound to the SparkSession " +
            s"instance for $user.")
        case (ss, _)
          ...
          info(s"Stopping the kyuubiAppMaster for [$user].")
          stopKyuubiAppMaster()
        case _ =>
      }
    }
  ...
  def getAmInstanceLock: InterProcessMutex = {
    val lockPath = KyuubiAppMasterWithUGI.getLockPath(user, conf)
  ...
  def stopKyuubiAppMaster(): Unit = {
    kyuubiAppMaster match {
      case Some(kam) =>
        val lock = getAmInstanceLock
        lock.acquire()
        info(s"Acquire the lock of KyuubiAppMaster for $user.")
        try {
          kam.offlineHaServiceFirst()
        } catch {
          case e: Exception =>
            error(s"Error occured when offline the service for $user.")
        } finally {
          lock.release()
          info(s"Release the lock of KyuubiAppMaster for $user.")
        }
        kam.stop(FinalApplicationStatus.SUCCEEDED,
          s"Stop the kyuubiAppMaster for idle more than ${idleTimeout / 1000} seconds.")
      case _ =>
    }
  }

  override def set(user: String, session: SparkSession): Unit = {
    sparkSession = Some(session)
    timesCount.set(1)
  }

  override def getAndIncrease(user: String): Option[SparkSession] = {
    (sparkSession, timesCount) match {
  ...
  override def decrease(user: String): Unit = {
    (sparkSession, timesCount) match {
      case (Some(ss), times) if !ss.sparkContext.isStopped =>
  ...
  override def init(conf: SparkConf): Unit = {
    idleTimeout = math.max(conf.getTimeAsMs(YARN_KYUUBIAPPMASTER_IDLE_TIMEOUT.key), 60 * 60 * 1000)
    kyuubiAmNamespace = conf.get(YARN_KYUUBIAPPMASTER_NAMESPACE.key)
    user = conf.get(YARN_KYUUBIAPPMASTER_USERNAME.key, "")
  ...
  override def start(): Unit = {
    ...
    info(s"Scheduling KyuubiAmSparkSession cache cleaning every $interval seconds")
  ...
  override def stop(): Unit = {
    info("Stopping KyuubiAmSparkSession Cache Manager")
