...
class KyuubiAppMasterWithUGI(
  ...
  private def makeAppMasterArgs: Array[String] = {
    ...
    args += "--conf"
    args += DRIVER_MEM + "=" + conf.getOption(YARN_KYUUBIAPPMASTER_MEMORY.key)
      .orElse(Some("10g")).get
    args += "--conf"
    args += DRIVER_EXTRA_JAVA_OPTIONS + "=" +
      conf.getOption(YARN_KYUUBIAPPMASTER_EXTRA_JAVAOPTIONS.key)
        .getOrElse("-verbose:gc -XX:+PrintGCDetails -XX:+PrintGCDateStamps " +
          "-XX:+PrintTenuringDistribution")
    args += "--conf"
    args += DRIVER_MEM_OVERHEAD + "=" +
      conf.getOption(YARN_KYUUBIAPPMASTER_MEMORY_OVERHEAD.key)
        .getOrElse("1g")
    args += "--conf"
    args += DRIVER_CORES + "=" + conf.getOption(YARN_KYUUBIAPPMASTER_CORES.key)
      .getOrElse("4")
    args += "--conf"
    args += "spark.driver.maxResultSize=" +
      conf.getOption(YARN_KYUUBIAPPMASTER_MAX_RESULTSIZE.key)
        .getOrElse(conf.getOption("spark.driver.maxResultSize").getOrElse("1g"))
  ...
  def init(): Unit = {
    ...
    instanceNamespace = kyuubiAmNamespacePrefix + "/" + KyuubiAppMasterWithUGI.uriPath
