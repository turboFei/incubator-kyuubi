...
import org.apache.kyuubi.beeline.hive.logs.BeelineInPlaceUpdateStream;
...
public class KyuubiCommands extends Commands {
  ...
  private Runnable createLogRunnable(final Statement statement,
    ...
    if (statement instanceof KyuubiStatement) {
      return new KyuubiLogRunnable(this, (KyuubiStatement) statement, DEFAULT_QUERY_PROGRESS_INTERVAL,
          eventNotifier);
    } else if (statement instanceof HiveStatement) {
  ...
  static class KyuubiLogRunnable implements Runnable {
    ...
    KyuubiLogRunnable(KyuubiCommands commands, KyuubiStatement kyuubiStatement,
  ...
  static class HiveLogRunnable implements Runnable {
    ...
    HiveLogRunnable(KyuubiCommands commands, HiveStatement hiveStatement,
                      long queryProgressInterval, InPlaceUpdateStream.EventNotifier eventNotifier) {
  ...
  private void showRemainingLogsIfAny(Statement statement) {
    if (statement instanceof KyuubiStatement) {
      ...
      beeLine.debug("The statement instance is n KyuubiStatement type: " + statement.getClass());
    }
  }
