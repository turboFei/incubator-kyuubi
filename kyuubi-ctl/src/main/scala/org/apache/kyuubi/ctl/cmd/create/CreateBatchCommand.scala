...
class CreateBatchCommand(cliConfig: CliConfig) extends Command[Batch](cliConfig) {
  ...
  def doRun(): Batch = {
    ...
    withKyuubiRestClient(normalizedCliConfig, map, conf) { kyuubiRestClient =>
      ...
      val normalizedRequestConf = Option(request.get("configs"))
        .map(_.asInstanceOf[HashMap[Object, Object]]).map { objMap =>
          objMap.asScala.map { case (k, v) =>
